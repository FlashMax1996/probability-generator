 ...
 * author chillurbrain
 */
public class ErlangaDistributionGenerator implements Generator {
    private static final double SHAPE = 2.0; // Форма (k)
    private static final double SCALE = 1.0; // Размер (theta)
    ...
    private GammaDistribution erlangaDistribution; // Закон распределения

    public ErlangaDistributionGenerator() {
        generatedRandomValues = new ArrayList<>();
        calculatedData = new ArrayList<>();
        erlangaDistribution = new GammaDistribution(SHAPE, SCALE);
    }

    public List<Double> getGeneratedRandomValues(int sampleSize) {
        for (int i = 0; i < sampleSize; i++) {
            generatedRandomValues.add(Double.valueOf(
                    String.format(Locale.US, "%.3f", erlangaDistribution.sample())
            ));
        }
    ...
    private double getChi2() {
        ...
        for (Integer key : getHistogramData().keySet()) {
            ...
            double p = erlangaDistribution.cumulativeProbability(
